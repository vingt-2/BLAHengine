...
bool Disc(const blaVec3& origin, const blaVec3& normal, blaF32 radius, const Ray& ray, blaVec3& outPosition, float& outT)
{
    ...
    RaycastHit hit = ray.RayToPlaneIntersection(origin, normal, outT);
...
void TranslationGizmoControl::Update()
{
    ...
    if (m_selectedAxis == NONE)
    {
        ...
        //TODO: Create a ScaledPosQuat so I dont have to do that ...
        ObjectTransform coneTransform;
        coneTransform.m_scale = coneVerticalScaling * halfArrowThickness;

        blaF32 closestT = MAX_NORMAL_FLOAT;

        coneTransform.m_posQuat = xT;
        coneTransform.GetScaledTransformMatrix(objectTransformForCollider);
        collider->setTransform(&(objectTransformForCollider[0][0]));
    ...
    else
    {
        ...
        blaVec3 camForward = camera->GetObjectTransform().GetRotation() * blaVec3(0.f, 0.f, 1.f);
        switch (m_selectedAxis)
        {
        case X_AXIS:
        {
            blaVec3 controlPlane = abs(glm::dot(camForward, objectZ)) > abs(glm::dot(camForward, objectY)) ? objectZ : objectY;
            RaycastHit h = screenRay.RayToPlaneIntersection(m_startingObjectOrigin, controlPlane, t);
        ...
        case CENTER_AXIS:
        {
            RaycastHit h = screenRay.RayToPlaneIntersection(m_startingObjectOrigin, -camForward, t);
    ...
    DebugDraw::DrawArbitraryGeometry(xT, coneVerticalScaling * halfArrowThickness, cone, blaVec4(color, 0.8f));
    ...
    DebugDraw::DrawArbitraryGeometry(yT, coneVerticalScaling * halfArrowThickness, cone, blaVec4(color, 0.8f));
    ...
    DebugDraw::DrawArbitraryGeometry(zT, coneVerticalScaling * halfArrowThickness, cone, blaVec4(color, 0.8f));
...
void ScaleGizmoControl::Update()
{
    ...
    else
    {
        ...
        {
            blaVec3 controlPlane = abs(glm::dot(camForward, objectZ)) > abs(glm::dot(camForward, objectY)) ? objectZ : objectY;
            RaycastHit h = screenRay.RayToPlaneIntersection(transform.GetPosition(), controlPlane, t);
        ...
        case CENTER_AXIS:
        {
            blaVec3 camUp = camera->GetObjectTransform().GetRotation() * blaVec3(0.f, 1.f, 0.f);
            blaVec3 camLeft = camera->GetObjectTransform().GetRotation() * blaVec3(1.f, 0.f, 0.f);
            RaycastHit h = screenRay.RayToPlaneIntersection(transform.GetPosition(), -camForward, t);
...
void RotationGizmoControl::Update()
{
    ...
    else
    {
        ...
        case X_AXIS:
        {
            RaycastHit h = screenRay.RayToPlaneIntersection(transform.GetPosition(), objectX, t);
        ...
        case Y_AXIS:
        {
            RaycastHit h = screenRay.RayToPlaneIntersection(transform.GetPosition(), objectY, t);
            transform.SetRotation(QuatBetweenVectors(glm::normalize(m_startingClickPosition - transform.GetPosition()), glm::normalize(h.hitPosition - transform.GetPosition())) * m_startingRotation);
            break;
        }
        case Z_AXIS:
        {
            RaycastHit h = screenRay.RayToPlaneIntersection(transform.GetPosition(), objectZ, t);
            transform.SetRotation(QuatBetweenVectors(glm::normalize(m_startingClickPosition - transform.GetPosition()), glm::normalize(h.hitPosition - transform.GetPosition())) * m_startingRotation);
            break;
        }
        default:
    ...
    blaVec3 cameraForward = camera->GetObjectTransform().GetRotation() * blaVec3(0, 0, 1);
