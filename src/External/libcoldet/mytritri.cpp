...
void multiple_tri_tri_intersect(const Triangle& t1, const Triangle& t2, Vector3D colPoints[], int faces[], int& size)
{
    ...
    if (td1.pointInTri(P21))
    {
        colPoints[size - 1] = P21;
        faces[size - 1] = 1;
        size++;
    }
    float t22 = -(p1.d + p2.d + c2 * n12) / ((b2 - c2)*n12);
    Vector3D P22(c2 + t22 * (b2 - c2));
    if (td1.pointInTri(P22))
    {
        colPoints[size - 1] = P22;
        faces[size - 1] = 1;
        size++;
    }
    {
    ...
    if (td2.pointInTri(P11))
    {
        colPoints[size - 1] = P11;
        faces[size - 1] = 2;
        size++;
    }
    float t12 = -(p1.d + p2.d + c1 * n12) / ((b1 - c1)*n12);
    Vector3D P12(c1 + t12 * (b1 - c1));
    if (td2.pointInTri(P12))
    {
        colPoints[size - 1] = P12;
        faces[size - 1] = 2;
        size++;
    }
    int a = 0;
