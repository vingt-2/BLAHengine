...
void Dualshock4::ControllerTest()
{
    ...
    {
        std::cout << "Pressed L3 ";
        std::cout << "Left Analog Stick: X:" << leftAnalog[0] << ", Y:" << leftAnalog[1] << "\n";
    }
    if (GetR1Button())
    {
        std::cout << "Pressed R1\n";
    ...
    {
        std::cout << "Pressed R3 ";
        std::cout << "Right Analog Stick: X:" << rightAnalog[0] << ", Y:" << rightAnalog[1] << "\n";
    }

    if (dpad[0] || dpad[1])
    {
        std::cout << "D PAD_ X: " << dpad[0] << " Y: " << dpad[1] << " \n";
    }

    SetLeftLFRumble(leftAnalog[0] < 0 ? abs(leftAnalog[0]) : 0);
    SetRightHFRumble(leftAnalog[0] > 0 ? abs(leftAnalog[0]) : 0);
    float intensity = 0.5 * (1.f + cosf(0.2f * m_time));
    SetLightBarColor(intensity * 0.5f * (1.f + cosf(m_time)), intensity *  0.5f* (1.f + cosf(2.f * m_time)), intensity *  0.5f* (1.f + cosf(0.5f * m_time)));
}
