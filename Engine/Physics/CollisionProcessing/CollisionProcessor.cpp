...
CollisionProcessor::CollisionProcessor(Timer* time, float* timestepPtr) :
    ...
    m_solveCount(0)
{
    m_bodiesList = blaVector<RigidBodyComponent*>();
...
void CollisionProcessor::BroadPhaseDetection()
{
    for (size_t i = 0; i < m_bodiesList.size(); i++)
    {
        ...
        // TODO: Colliders aren't officially defined as colliders yet because of polymorphism, can't GetComponent them ... :/
...
{
    //blaMat4 t1 = body1->GetTransformComponent().m_posQuat;
    //body1->m_collider->m_collisionMesh->setTransform(&t1[0][0]);

    //blaMat4 t2 = body2->GetTransformComponent().m_posQuat;
    //body2->m_collider->m_collisionMesh->setTransform(&t2[0][0]);

    //blaVector<blaPair<int, int>> collidingTriangles;
    //blaVector<float> collisionPoints;
    //blaVector<int> collidingFaces;
    //blaVector<int> colSizes;
    ...
    //        blaVec3 contactInBody1 = body1->GetTransformComponent().WorldPositionToLocal(collisionPoint);
    //        blaVec3 contactInBody2 = body2->GetTransformComponent().WorldPositionToLocal(collisionPoint);
    ...
    //        blaVec3 normalBody1W = body1->GetTransformComponent().LocalDirectionToWorld(body1ContactNormal);
    //        blaVec3 normalBody2W = body2->GetTransformComponent().LocalDirectionToWorld(body2ContactNormal);


    //        blaVec3 tangentBody1W = body1->GetTransformComponent().LocalDirectionToWorld(body1ContactTangent);
    //        blaVec3 tangentBody2W = body2->GetTransformComponent().LocalDirectionToWorld(body2ContactTangent);
    ...
    //            blaVec3 body1to2 = body2->GetTransformComponent().GetPosition() - collisionPoint;
    ...
    //            blaVec3 body2to1 = body1->GetTransformComponent().GetPosition() - collisionPoint;
...
{
    blaVector<blaVector<blaVec3>> T(3 * m_currentContacts.size());
    ComputeT(T);

    blaVector<float> D(3 * m_currentContacts.size());
    ...
    for (size_t i = 0; i < m_currentContacts.size(); i++)
    {
        ...
        blaScaledTransform body1Transform = body1->GetOwnerObject().GetComponent<TransformComponent>()->GetTransform();
        blaVec3 vLUpdate1 = (blaVec3)body1->m_nextState.m_velocity;
        blaVec3 vAUpdate1 = (blaVec3)body1Transform.LocalDirectionToWorld(body1->m_nextState.m_angularVelocity);
        RigidBodyComponent* body2 = contact.m_body2;
        blaScaledTransform body2Transform = body2->GetOwnerObject().GetComponent<TransformComponent>()->GetTransform();
        blaVec3 vLUpdate2 = (blaVec3)body2->m_nextState.m_velocity;
        blaVec3 vAUpdate2 = (blaVec3)body2Transform.LocalDirectionToWorld(body2->m_nextState.m_angularVelocity);

        blaVector<blaVec3> nJac = contact.m_normalJacobian;
        blaVector<blaVec3> t1Jac = contact.m_tangentJacobian1;
        blaVector<blaVec3> t2Jac = contact.m_tangentJacobian2;
    ...
    blaVector<int> contactIndices(m_currentContacts.size());
    for (size_t i = 0; i < m_currentContacts.size(); i++)
    {
        contactIndices[i] = i;
    }
    ...
    while (iteration < m_maxIterations && averageDistance > m_epsilon)
    {
        ...
        for (auto index : contactIndices)
        {
            for (int i = 0; i < 3; i++)
            {
                ...
                blaVector<blaVec3>* jacobianEntry;
                if (i == 0)
...
void CollisionProcessor::ComputeT(blaVector<blaVector<blaVec3>>& T)
{
...
void CollisionProcessor::GetDiagonalElements(blaVector<blaVector<blaVec3>> T, blaVector<float>& D)
{
    for (size_t i = 0; i < 3 * m_currentContacts.size(); i++)
    {
        Contact contact = m_currentContacts[(int)(i / 3)];
...
void Contact::ComputeJacobian()
{
    ...
    blaScaledTransform body1Transform = m_body1->GetOwnerObject().GetComponent<TransformComponent>()->GetTransform();
    blaScaledTransform body2Transform = m_body2->GetOwnerObject().GetComponent<TransformComponent>()->GetTransform();

    blaVec3 radialBody1 = m_contactPositionW - body1Transform.GetPosition();
    blaVec3 radialBody2 = m_contactPositionW - body2Transform.GetPosition();
