...
public:
    ...
    */
    ...
    virtual bool getCollidingTriangles(std::vector<std::pair<int, int>>* pairs) = 0;
...
*/
BLACORE_API  CollisionModel3D* newCollisionModel3D(bool Static = false);
...
*/
BLACORE_API  bool SphereRayCollision(const float* center, float radius,
    const float* origin, const float* direction,
    float* point);

/** Checks for intersection between 2 spheres. */
BLACORE_API  bool SphereSphereCollision(float c1[3], float r1,
