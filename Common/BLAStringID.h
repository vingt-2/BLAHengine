...
public:
    ...
    operator blaU32() const
    {
        return m_crc;
    }

    blaBool operator==(const blaStringId& other) const
    {
...
#define BlaStringId(str) blaStringId(str, COMPILE_TIME_CRC32_STR(str))

inline blaU32  _crc32(const char* buf, blaU32 len)
{
    uint8_t octet;
    const char* p, * q;

    uint32_t crc = 0xFFFFFFFF; // or crc = 0 -> crc = ~crc
    q = buf + len;
    for (p = buf; p < q; p++) {
...
inline blaStringId FromString(blaString string)
{
    ...
    if(i != blaString::npos) 
    {
        blaString name = string.substr(0, i);
        blaString crc = string.substr(i + 1, string.size() - i);
        return blaStringId(string.substr(0, i), std::stoul(crc));
    }
    return blaStringId("", 0);
}

inline blaString ToString(blaStringId stringId)
{
    return blaString(stringId) + ":" + std::to_string((blaU32)stringId);
}